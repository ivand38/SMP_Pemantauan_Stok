...
class DetailSalesProdukService {
  var logger = Logger();
  String baseUrl = 'https://79ff-36-65-189-223.ngrok-free.app/api';

  Future<List<DetailSalesProdukModel>> getDetailSalesProduk(
      String token) async {
    ...
    if (response.statusCode == 200) {
      List data = json.decode(response.body)['data']['produk'];
      List<DetailSalesProdukModel> detailSalesProduks = [];

      for (var item in data) {
        detailSalesProduks.add(DetailSalesProdukModel.fromJson(item));
      }

      return detailSalesProduks;
    } else {
      List<DetailSalesProdukModel> detailSalesProduks = [];
      return detailSalesProduks;
    }
